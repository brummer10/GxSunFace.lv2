...
#define GTK_SUNFACE_KNOB_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), GTK_TYPE_SUNFACE_KNOB, GtkSunFaceKnobPrivate))

G_DEFINE_TYPE(GtkSunFaceKnob, gtk_knob, GTK_TYPE_RANGE);

typedef struct _GtkSunFaceKnobPrivate GtkSunFaceKnobPrivate;

struct _GtkSunFaceKnobPrivate
{
    int knob_x;
    int knob_y;
    int knob_step;
...
static void knob_expose(GtkWidget *widget, int knob_x, int knob_y, GdkEventExpose *event, int arc_offset)
{
    /** check resize **/
    int grow;
    GtkSunFaceKnobPrivate *priv = GTK_SUNFACE_KNOB_GET_PRIVATE(widget);
    ...
    if (priv->model != 3) {
        ...
        GtkSunFaceKnob *knob = GTK_SUNFACE_KNOB(widget);
...
{
    g_assert(GTK_IS_SUNFACE_KNOB(widget));
    GtkSunFaceKnob *knob = GTK_SUNFACE_KNOB(widget);
    GtkSunFaceKnobPrivate *priv = GTK_SUNFACE_KNOB_GET_PRIVATE(knob);
...
{
    GtkSunFaceKnob *knob = GTK_SUNFACE_KNOB(object);
...
/****************************************************************
 ** init the GtkSunFaceKnobClass
 */

static void gtk_knob_class_init (GtkSunFaceKnobClass *klass)
{
...
//--------- connect the events with funktions
    ...
    g_type_class_add_private(obj_class, sizeof (GtkSunFaceKnobPrivate));
}
...
static void gtk_knob_init (GtkSunFaceKnob *knob)
{
...
{
    GtkWidget *widget = GTK_WIDGET( g_object_new (GTK_TYPE_SUNFACE_KNOB, NULL ));
